...
def load_radar(example_path: AnyStr) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    radar_resolution = np.array([0.17361], np.float32)
    encoder_size = 5600
    #raw_example_data = imageio.imread(example_path)

    raw_example_data = example_path
    ...
    return fft_data, radar_resolution
...
def radar_polar_to_cartesian(azimuths: np.ndarray, fft_data: np.ndarray, radar_resolution: float,
    ...
    print(sample_angle.shape, azimuths[0].shape, azimuth_step.shape)
