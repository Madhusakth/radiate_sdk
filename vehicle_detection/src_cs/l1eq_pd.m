...
while (~done)
  ...
  if (largescale)
    ...
    opts.POSDEF = true; opts.SYM = true;  %%%%%%%%%%%%%%%%%%%%%%%
    if sum(isinf(H11p(:)))>0
      disp('Matrix has inf.  Returning previous iterate.  (See Section 4 of notes for more information.)');
      xp = x;
      return
    end
    if sum(isnan(H11p(:)))>0
      disp('Matrix has nan.  Returning previous iterate.  (See Section 4 of notes for more information.)');
      xp = x;
      return
    end
    %%%%%%
    ...
    if all(val>0)
        ...
        H11p = H11p + 1e-8*eyes(length(H11p));
    end
