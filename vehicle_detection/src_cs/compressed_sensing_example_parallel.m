...
%___INPUT IMAGE___
function x_return = compressed_sensing_example(A, w, h, rate, Phi)
% w = w;
% h = h;
x = double(A(:));
n = length(x);
...
%m = m ; %int16((w*h)/2); % NOTE: small error still present after increasing m to 1500;
...
% subplot(1,2,1), imagesc(reshape(x,w,h)), xlabel('original'), axis image
% %subplot(1,3,2), imagesc(reshape(x2,w,h)), xlabel('least squares'), axis image
% subplot(1,2,2), imagesc(reshape(x1,w,h)), xlabel('basis pursuit'), axis image
% colormap gray
x_return = reshape(x1,w,h);
%disp(x_return)
return
end
